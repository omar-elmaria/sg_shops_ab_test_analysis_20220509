...
# V2 contains logic to calculate "Treatment_Scope"
...
target_group_names_tgx_non_tg <- c("TG1", "TG2", "TG3", "TG4", "Non_TG") # Names of the target groups that should be filtered for in the SQL query. Always follow a chronological order
target_group_names_ts <- c("TG1", "TG2", "TG3", "TG4") # Names of the target groups that should be filtered for in the SQL query. Always follow a chronological order
...
col_order <- c("entity_id", "experiment_id", "variable", # Grouping variables
               ...
               # Group = TG3
               "TG3_Control", "TG3_Var1", "Delta_Pct_V1_C_TG3", "Delta_Abs_V1_C_TG3", "P_Val_V1_C_TG3", 
               "TG3_Var2", "Delta_Pct_V2_C_TG3", "Delta_Abs_V2_C_TG3", "P_Val_V2_C_TG3",
               
               # Group = TG4
               "TG4_Control", "TG4_Var1", "Delta_Pct_V1_C_TG4", "Delta_Abs_V1_C_TG4", "P_Val_V1_C_TG4", 
               "TG4_Var2", "Delta_Pct_V2_C_TG4", "Delta_Abs_V2_C_TG4", "P_Val_V2_C_TG4",
               
               # Group = Treatment_Scope
               "TS_Control", "TS_Var1", "Delta_Pct_V1_C_TS", "Delta_Abs_V1_C_TS", "P_Val_V1_C_TS", 
               "TS_Var2", "Delta_Pct_V2_C_TS", "Delta_Abs_V2_C_TS", "P_Val_V2_C_TS",
               
               # Group = TGx + Non-TG
               "TGx_Non_TG_Control", "TGx_Non_TG_Var1", "Delta_Pct_V1_C_TGx_Non_TG", "Delta_Abs_V1_C_TGx_Non_TG", "P_Val_V1_C_TGx_Non_TG",
               "TGx_Non_TG_Var2", "Delta_Pct_V2_C_TGx_Non_TG", "Delta_Abs_V2_C_TGx_Non_TG", "P_Val_V2_C_TGx_Non_TG")

# Combined target groups
combined_tgx_non_tg <- "TGx_Non_TG" # If target groups need to be combined, specify the new target group designation here
combined_ts <- "TS"

# Target group order
tg_order <- c("Non_TG", "TG1", "TG2", "TG3", "TG4", "TS", "TGx_Non_TG")

# Number of cvr_metrics calculated in df_cvr_dwnld
num_cvr_metric = 4 # 3 for CVR, CVR2, CVR3, and mCVR4

# BQ table name
bq_tbl_name <- "loved_brands_final_results_business_kpis_sg_shops_v2"
...
# Orders data
df_ord <- tbl(bq_conn, "ab_test_individual_orders_cleaned_loved_brands_sg_shops") # Contains business and logistical KPIs of the LB experiment(s)

# CVR data (NEEDS MODIFICATION)
df_cvr_overall <- tbl(bq_conn, "ab_test_cvr_data_cleaned_loved_brands_sg_shops_overall") # Contains conversion and user count data (Overall)
df_cvr_per_day <- tbl(bq_conn, "ab_test_cvr_data_cleaned_loved_brands_sg_shops_per_day") # Contains conversion and user count data (Per Day)

##-----------------------------------------------------END OF STEP 2 AND INPUT SECTION-----------------------------------------------------##

# Step 3: Calculation of the correction factors based on the user counts

# Create a function to extract the user count data from df_cvr_overall
...
results_func <- function(raw_ord_df, tg_incl_filter_var, tg, combined_tgs, per_day, corr_factor_overall, corr_factor_per_day, exp_id, entity) {
  if (combined_tgs == 'Yes') {
    df_temp <- raw_ord_df %>%
...
# Overall (i.e. no grouping by created_date) with individual target groups
# ***FUNCTION CALLING***
df_overall_with_tg <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = NULL, tg = NULL, combined_tgs = "No", per_day = "No", 
                                   corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)

# df_overall without individual target groups
# First: TGx + Non-TG
df_overall_tgx_non_tg <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = target_group_names_tgx_non_tg, tg = combined_tgx_non_tg, combined_tgs = "Yes", per_day = "No", 
                                      corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)

# Second: TS (All TG(x) combined into one)
df_overall_ts <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = target_group_names_ts, tg = combined_ts, combined_tgs = "Yes", per_day = "No", 
                              corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)

# Combine df_overall_with_tg and df_overall_tgx_non_tg
...
delta_calc_func <- function(df) {
  ...
  for (i in variation_col_names) {
    ...
    # Breakdown of the regex below --> # \w matches any word character (equivalent to [a-zA-Z0-9]). + matches the previous token between one and unlimited times, as many times as possible, giving back as needed (greedy). (?=_Variation[0-9]) --> Positive Look-ahead
    ...
    df <- df %>%
      dplyr::mutate(Delta_Pct = round((!!as.name(i) - !!as.name(paste0(tg_associated_with_col, "_Control"))) / abs(!!as.name(paste0(tg_associated_with_col, "_Control"))), 4), # (Var - Control) / ABS(Control) to account for changes in negative values
...
# Per Day with individual target groups
# ***FUNCTION CALLING***
df_per_day_with_tg <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = NULL, tg = NULL, combined_tgs = "No", per_day = "Yes", 
                                   corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)

# df_per_day without individual target groups
# First: TGx + Non-TG
df_per_day_tgx_non_tg <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = target_group_names_tgx_non_tg, tg = combined_tgx_non_tg, combined_tgs = "Yes", per_day = "Yes", 
                                      corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)

# Second: TS (All TG(x) combined into one)
df_per_day_ts <- results_func(raw_ord_df = df_ord, tg_incl_filter_var = target_group_names_ts, tg = combined_ts, combined_tgs = "Yes", per_day = "Yes", 
                              corr_factor_overall = df_corr_factor_overall, corr_factor_per_day = df_corr_factor_per_day, exp_id = exp_id_var, entity = ent_id)
  ...
  for (x in 1:length(levels(df_per_day$target_group))) { # Loops over the different df_pval columns
    for (i in 1:ncol(df_per_day)) { # Loops over the different columns in df_per_day
      if (colnames(df_per_day[, i]) %in% c("entity_id", "experiment_id", "created_date", "variant", "target_group", "corr_factor") | any(is.na(df_per_day[, i]))){ # HERE: Conditions could be changed
        ...
        df_pval[i-5, j] <- round(wilcox.test( # "i-5" because the first 5 columns in df_per_day will be skipped
...
df_pval_biz_kpis_all <- df_pval_biz_kpis_all[, c("Var_Pair", colnames(one_random_df_pval_biz)[1:(length(one_random_df_pval_biz)-1)])]

# Add the prefix "P_Val" to the column names
col_names_with_tg <- colnames(df_pval_biz_kpis_all)[grep("TG|TS", colnames(df_pval_biz_kpis_all))]
colnames(df_pval_biz_kpis_all)[grep("TG|TS", colnames(df_pval_biz_kpis_all))] <- sapply(col_names_with_tg, function(x) {paste0("P_Val_", x)})
...
p_val_join_func <- function(df_pval, df_kpis) {
  for (i in sort(unique(df_pval$Var_Pair))) {
    ...
    df_pval_col_name_sep <- sapply(colnames(y)[grep("P_Val", colnames(y))], function(x) {strsplit(x, "P_Val")[[1]][2]}) # This function separates "P_Val_TG1_Non_TG" to "nothing" and "_TG1_Non_TG". It applies this manipulation to all columns that have "P_Val" in their name
    df_pval_col_name_new <- sapply(df_pval_col_name_sep, function(y, x) {paste0("P_Val_", str_sub(x, 1, 4), y)}, x = i) # First argument of the function f(x) always refers to the first argument of the apply function
    
    colnames(y)[grep("P_Val", colnames(y))] <- df_pval_col_name_new # Rename the columns that contain "P_Val" in their name to "df_pval_col_name_new"
...
contingency_tbl_func <- function(per_day, exp_id, entity) {
  ...
  # Filter the CVR table for the right entity_id and experiment_id
  df_cvr_dwnld <- cvr_df %>% 
    filter(experiment_id == exp_id, entity_id == entity) %>%
    collect()
  
  df_cvr_conting <- df_cvr_dwnld %>% # Contingency table for significance calculation
    select(entity_id, experiment_id, target_group, variant, transactions, shop_list_sessions, shop_menu_sessions, checkout_sessions, checkout_transaction_sessions, total_sessions) # These columns can be used to calculate CVR2, CVR3, and mCVR4
...
# Step 10: Calculate the significance of CVR, CVR2, CVR3, and mCVR4. Create a function that calculates the p-values of these CVRs (several steps involved)
...
pval_calc_cvr_func <- function(tg, variation, cvr_metric, cvr_df, exp_id, entity) {
  cvr_df <- cvr_df %>% 
    ...
    {if (cvr_metric == "CVR2") 
      select(., entity_id, experiment_id, target_group, variant, transactions, shop_list_sessions) 
        ...
        select(., entity_id, experiment_id, target_group, variant, checkout_transaction_sessions, checkout_sessions)
      else if (cvr_metric == "CVR")
        select(., entity_id, experiment_id, target_group, variant, transactions, total_sessions)
    }
  ...
  if (cvr_metric == "CVR2") {
    ...
    return(round(prop.test(x = cvr_df$checkout_transaction_sessions, n = cvr_df$checkout_sessions, alternative = "two.sided", correct = FALSE)$p.value, 4)[[1]])
  } else if (cvr_metric == "CVR") {
    return(round(prop.test(x = cvr_df$transactions, n = cvr_df$total_sessions, alternative = "two.sided", correct = FALSE)$p.value, 4)[[1]])
  }
}
...
pval_tbl_cvr_func <- function(cvr_conting, cvr_dwnld, num_of_cvr_metrics, variation, var_pair_num, exp_id, entity){
  ...
  for (j in levels(cvr_conting$target_group)) {
    for (i in row.names(df_pval_tbl)) {
      if (!(j %in% c("TGx_Non_TG", "TS")) & i %in% c("CVR", "CVR2")) { # Don't calculate p-values when the CVR metric = CVR or CVR2 and the TG is NOT TS or TGx_Non_TG
        df_pval_tbl[i,j] <- NA
      } else {
        df_pval_tbl[i,j] <- pval_calc_cvr_func(tg = j, variation = variation, cvr_metric = i, cvr_df = df_cvr_conting, exp_id = exp_id, entity = entity) # df_cvr_conting is the actual dataframe
      }
    }
  }
...
# ***FUNCTION CALLING***
for (i in 1:num_variants) {
  assign(paste0("df_pval_cvr_v", i, "_c"), 
         pval_tbl_cvr_func(cvr_conting = df_cvr_conting, cvr_dwnld = df_cvr_dwnld, num_of_cvr_metrics = num_cvr_metric, 
                           variation = paste0("Variation", i), var_pair_num = i, exp_id = exp_id_var, entity = ent_id)) # 2 for CVR2 and CVR3. If you have more CVRs or proportions, increase the number
}
...
# Add the prefix "P_Val" to the column names
colnames(df_pval_cvr_all)[grep("TG|TS", colnames(df_pval_cvr_all))] <- sapply(col_names_with_tg, function(x) {paste0("P_Val_", x)}) # col_names_with_tg comes from the df_pval_biz_kpis section
...
tg3_orders <- df_ord %>% 
  filter(target_group == "TG3") %>% 
  collect()

ggplot(tg3_orders, aes(x = actual_df_paid_by_customer, fill = variant)) +
  geom_histogram(boundary = 0, closed = "right", binwidth = 0.25) +
  # stat_bin(geom = "text", binwidth = 0.25, aes(label = ..count..), breaks = seq(0, 6, 0.25)) + # This or the one below it
  geom_text(stat = "bin", binwidth = 0.25, aes(label = ..count..), breaks = seq(0, 6, 0.25), vjust = -0.25) +
